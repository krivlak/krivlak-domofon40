...
namespace domofon40
{
    public partial class горизонт1дом : Form
    {
        public горизонт1дом()
        {
            InitializeComponent();
       ...
       // List<temp> tempList0 = new List<temp>();
        private void горизонт1дом_Load(object sender, EventArgs e)
        {
            string curDir = System.IO.Directory.GetCurrentDirectory();

            string шаблон = curDir + @"\горизонт1дом.sql";
        ...
        //    sb.AppendLine("declare @вид uniqueidentifier = '" + клВид_услуги.вид_услуги.ToString() + "' ; ");
            ...
            var query = de.оплачено
                ...
                .Select(n => new { n.месяц, n.оплаты.клиент, n.услуга, n.сумма })
                .ToDictionary(n => (n.клиент, n.услуга, n.месяц));

            bool цвет = false;
            Guid код_клиента = Guid.Empty;
            foreach (temp tRow in tempList)
            {
                if(tRow.клиент!= код_клиента)
                {
                  цвет = ! цвет;
                }
                ...
                for (int m = 1; m < 13; m++)
                {
                    var ключ = (tRow.клиент, tRow.услуга, m);
                    if (query.ContainsKey(ключ))
                    {
                        var uRow = query[ключ];
                        tRow.setField(m, uRow.сумма);
                    }
                }
                код_клиента = tRow.клиент;
            }
       ...
       //     обновить_дом();
        ...
        //   static  int[] массив = new int[12];

            public void setField(int месяц, int оплачено)
            {
                
                //массив[месяц-1] = оплачено;
                //                int caseSwitch = 1;
                switch (месяц)
                {
                    case 1:
