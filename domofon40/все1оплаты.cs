...
namespace domofon40
{
    public partial class все1оплаты : Form
    {
        public все1оплаты()
        {
            InitializeComponent();
        ...
        private void все1оплаты_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string шаблон = curDir + @"\все1оплаты.sql";
            ...
            //    tempList = de.Database.SqlQuery<temp>("все1оплаты @сотрудник= @p0", клСотрудник.сотрудник).ToList();
            ...
            FormClosed += Все1оплаты_FormClosed;
        }
        ...
        void temp_Moving(temp obj)
        {
            ...
            if (obj.поле == "прим0")
            {
                ...
                de.SaveChanges();
                //temp.Moving -= temp_Moving;
                //tempList.FindAll(n => n.клиент == kRow.клиент).ForEach(n => n.прим0 = obj.прим0);
                //temp.Moving += temp_Moving;
            }
            if (obj.поле == "телефон")
            {
        ...
        private void Все1оплаты_FormClosed(object sender, FormClosedEventArgs e)
        {
            temp.Moving -= temp_Moving;
        }
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            if (клКлиент.выбран)
            {
                ...
                aPar[5] = new SqlParameter("@вид_оплаты", vRow.вид_оплаты);
                try
                {
                    int строк = de.Database.ExecuteSqlCommand(sqlString, aPar);
                    //MessageBox.Show(de.оплаты.Max(n => n.номер).ToString());  // видимо de.Database.ExecuteSqlCommand обновляет de
                    if (строк < 1)
                    {
                        MessageBox.Show("Ошибка записи " + строк.ToString());
                    }
                ...
                //newOp.дата = NewRow.дата;
                //newOp.клиент = NewRow.клиент;
                //newOp.номер = NewRow.номер;
                //newOp.оплата = NewRow.оплата;
                //newOp.сотрудник = NewRow.сотрудник;
                //try
                //{
                //    de.оплаты.Add(newOp);
                //    de.SaveChanges();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (bindingSource1.Count > 0)
            {
                ...
                if (xy == System.Windows.Forms.DialogResult.Yes)
                {
                    ...
                    //       string sqlString = "delete from оплаты where оплата='" + uRow.оплата.ToString() + "'";
                    //  string sqlString = "delete from оплаты where оплата=@оплата" ;
                    //  int строк = de.Database.ExecuteSqlCommand(sqlString);
                    //     SqlParameter par = new SqlParameter("@оплата", uRow.оплата);

                    //int строк = de.Database.ExecuteSqlCommand(sqlString, par);
                    try
                    {
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            if (bindingSource1.Count > 0)
            {
                ...
                выборУслуги.Text = "Выберите услугу " + uRow.адрес.Trim() + " " + uRow.фио;
                выборУслуги.ShowDialog();
                if (клУслуга.выбран)
                {
                    Cursor = Cursors.WaitCursor;
                    оплаченые1месяца оплаченыеМесяца = new оплаченые1месяца();
                    оплаченыеМесяца.Text="Оплачено за "+клУслуга.deRow.наимен.Trim()+" "+ uRow.адрес.Trim() + " " + uRow.фио;
        ...
        private void button11_Click(object sender, EventArgs e)
        {
            if (bindingSource1.Count > 0)
            {
                ...
                //List<клиенты> клиентыЛист = de.клиенты.Where(n => n.клиент == клКлиент.клиент).ToList();
                //сведенияКлиента.bindingSource1.DataSource = клиентыЛист;
